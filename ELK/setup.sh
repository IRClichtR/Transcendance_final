...
/usr/share/wait-for-it.sh elasticsearch:9200 -t 60

# Create Kibana user
echo "Creating Kibana user..."
curl -u elastic:changemeoupas -X POST "http://elasticsearch:9200/_security/user/kibana_user" -H "Content-Type: application/json" -d '
{
  "password" : "changemeoupas",
  "roles" : [ "kibana_system" ],
